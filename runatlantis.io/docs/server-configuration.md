...
Enable external storage backends configured in the server-side repo config (`external_stores` block). When set, Atlantis reads the `external_stores` section from the repo config YAML to initialize backends such as S3 for plan file persistence. See [ExternalStores](server-side-repo-config.md#externalstores) for the full syntax.
